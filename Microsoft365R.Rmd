...
## 複数のOffice文書の文字列を一括置換

毎年同じような文書を作成しているが，年だけを更新しなければならないことは多いだろう．
WordやExcelの検索・置換の機能を用いれば，1つのファイル内の文字列を置換できる．
また，VBAでマクロを使えば，複数ファイルの文字列置ができる．
その場合，1つの組み合わせだけでなく，複数の組み合わせの置換もできるだろう．

VBAのマクロと同様のことをRから実行してみる．
さらに，日付のデータがある場合は，パッケージlubridateを利用して曜日の整合性をチェックする．
日付を変更して曜日を変更し忘れる，あるいは日付を変更し忘れることは多くの人がやってしまいがちだ．
日付(月日)を固定したい場合であれば，曜日のみ変更すれば良いので分かりやすい．
曜日を固定したい場合は「10月の第2日曜日」のように法則がきっちりとしていれば，プログラムは簡単である．
あるいは，10月1日の前後3日以内の火曜日のような法則でも可能である．
とにかく，決め方が明確であり，それによって1つに日付を決めることができれば，プログラムによる自動化ができる．
...
wd <- "D:/matu/work/tmp/"
setwd(wd)
path <- "2023卒業研究について.docx"
doc <- officer::read_docx(path)
doc <- officer::headers_replace_all_text(doc, old_value = "卒業", new_value = "そつぎょう", warn = FALSE)
doc <- officer::footers_replace_all_text(doc, old_value = "卒業", new_value = "そつぎょう", warn = FALSE)
doc <- officer::body_replace_all_text(doc, old_value = "卒業", new_value = "そつぎょう")
print(doc, target = "2023.docx")
